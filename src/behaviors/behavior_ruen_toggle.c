#define DT_DRV_COMPAT zmk_behavior_ruen_toggle
...
struct behavior_ruen_toggle_config {
...
static int on_ruen_toggle_pressed(struct zmk_behavior_binding *binding, struct zmk_behavior_binding_event event) {
    return ZMK_BEHAVIOR_OPAQUE;
}

static int on_ruen_toggle_released(struct zmk_behavior_binding *binding, struct zmk_behavior_binding_event event) {
    const struct device *dev = zmk_behavior_get_binding(binding->behavior_dev);
    const struct behavior_ruen_toggle_config *cfg = dev->config;
...
static const struct behavior_driver_api behavior_ruen_toggle_driver_api = {
    .binding_pressed = on_ruen_toggle_pressed,
    .binding_released = on_ruen_toggle_released,
};

#define RUEN_TOGGLE_INST(n)                                                             \
    static const struct behavior_ruen_toggle_config behavior_ruen_toggle_config_##n = { \
    ...
    BEHAVIOR_DT_INST_DEFINE(n, NULL, NULL, NULL, &behavior_ruen_toggle_config_##n, POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &behavior_ruen_toggle_driver_api);
DT_INST_FOREACH_STATUS_OKAY(RUEN_TOGGLE_INST)
